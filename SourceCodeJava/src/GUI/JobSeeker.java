...
public class JobSeeker extends JFrame implements WindowListener {
	...
	public JobSeeker(Account user) {
		...
		tableJ = new JTable();
		String[] nameOfColumns = { "EMPLOYER_NAME", "CATEGORY_JOB_NAME", "JOB_NAME", "COMPANY_NAME", "CITY", "SALARY",
				"DESCIPTION_JOB", "LABOR" };
		...
		btnCancelApplieds.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (tableApplied.getSelectedRowCount() > 0) {
					...
					BLL_GUEST.getInstance().CancelAppliedsI_BLL_GUEST(
							tableApplied.getModel().getValueAt(tableApplied.getSelectedRow(), 9).toString());
