...
public class BLL_LOGIN {
	...
	public boolean Register(String NAME, String CITY, String PHONE, String EMAIL, String AGE, String PROFESSIONAL,
			boolean GENDER, String USERNAME, String PASSWORD, String CONFIRM) {
		...
		if (NAME.length() != 0 && CITY.length() != 0 && PHONE.length() != 0 && EMAIL.length() != 0 && AGE.length() != 0
			...
			if (PASSWORD.equals(CONFIRM)) {
				if (checkPhone(PHONE) && checkEmail(EMAIL)) {
					...
					String queryA = insertA + IDA + "','" + USERNAME + "','" + PASSWORD + "','" + accesser + "','"
							+ status + "')";
					...
					if (LogIn.p == permission.JobSeeker && checkAge(AGE)) {
						...
						query = insert + ID + "','" + IDA + "','" + IDP + "'," + Integer.parseInt(AGE) + ",'" + GENDER
								+ "','" + PROFESSIONAL + "')";
					} else if (LogIn.p == LogIn.permission.Employer) {
